...
export default function UserContests() {
  ...
  const fetchMyPools = async () => {
    ...
    try {
      ...
      const data: Pool[] = await getMyPools(matchId as string);
      setMyPools(data);
      // console.log(data);
      setError(null);
    } catch (error) {
  ...
  const handleIncompleteQuiz = (pool_id: string | number) => {
    if (pool_id) {
      router.push(`/place-bet?initPoolId=${pool_id}`);
    }
  };
          ...
          <div key={betSize} className="mb-4 bg-[#2365EA] rounded-lg">
            <div className="flex text-center p-4 text-sm justify-between">
              ...
              <div className="flex flex-col">
                <h1>Spots</h1>
                <h1>
                  {pools[0].max_participants} Pools |{" "}
                  {pools.reduce(
                    (acc, pool) => acc + pool.current_participants,
                    0
                  )}{" "}
                  Players
                </h1>
            ...
            {pools.map((pool) => (
              <div
                key={pool.id}
                className="flex justify-between text-[12px] p-2 bg-slate-900 rounded-b-lg border-b-[1px]"
              >
                <h1>Pool {pool.id}</h1>
                <h1>
                ...
                {pool?.quiz_progress?.completion_percentage !== 100 ? (
                  <button
                    className="bg-yellow-500 text-black text-xs hover:bg-yellow-600 rounded p-1 "
                    onClick={() => {
                      handleIncompleteQuiz(pool?.id);
                    }}
                  >
                    Incomplete Quiz
                  </button>
